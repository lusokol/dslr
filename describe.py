...
def displayStats(stats):
    df = pd.DataFrame(stats, columns=[' ', 'Count', 'Mean', 'Std', 'Variance', 'Min', '25%', '50%', '75%', 'Max'])
...
def main():
    ...
    parser.add_argument("dataset", type=str, help="input dataset to describe")
    ...
    for col in dataset.columns[6:]: # we only want usefull data
        ...
        line.append(variance(dataset[col].values))
